...
var s3tc = module.exports = {
  ...
  s3Cache: {
    ...
    remove: function(key, callback) {
      s3tc.s3.deleteObject({Key: s3tc.s3Cache.internalKey(key)}, callback);
    },
  ...
  afterPhantomRequest: function(req, res, next) {
    ...
    } else if (pre.statusCode === 404 || pre.statusCode === 410) {
      this.s3Cache.remove(pre.url, function(err, result) {
        console.error('remove cached entry', pre.url, pre.statusCode);
      });
    }
